...
void traverse(char *t,int (*func)()) //this is a "depth-first" tranversal
{
  struct NODE *s = (struct NODE*)t; //s is a struct that was passes in to traverse

  if(s->left != NULL)traverse((char*)s->left,func); //if there is no left-child, then call traverse with member left of struct s
