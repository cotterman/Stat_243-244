  ...
  int n,p,q;
{
  ...
  alpha = fmalloc(r);
  for(i=0;i<r;i++)alpha[i] = i < p ? (float)a[i] : 0.;
  beta = fmalloc(r);
  for(i=0;i<r;i++)beta[i] = i < q ? (float)b[i] : 0.;

  x = fmalloc(n);
  for(i=0;i<n;i++)x[i] = (float)xd[i];
  ...
  k = 0;
  for(i=0;i<p;i++)parms[k++] = a[i];
  for(i=0;i<q;i++)parms[k++] = b[i];

  //return a large likelihood value if parameters are illegal
  ...
  if(p == 1 && q == 0){
	...
	*pp = 1 / (1 - alpha[0] * alpha[0]);
	...
	upd = 1;
	starma_(&p,&q,&r,&np,alpha,beta,aa,pp,v,thetab,xnext,xrow,rbar,
		&nrbar,&ifault);
	if(ifault)printf("after starma, ifault=%d\n",ifault);
	}

  /*printf("after starma, alpha=");
  for(i=0;i<r;i++)printf(" %g",alpha[i]);
  printf("\n              beta=");
  for(i=0;i<r;i++)printf(" %g",beta[i]);
  printf("\n");*/

  resid = fmalloc(n);
  e = fmalloc(r);
  ssq = sumlog = 0.;
  karma_(&p,&q,&r,&np,alpha,beta,aa,pp,v,&n,x,resid,&sumlog,&ssq,&upd,
	 &delta,e,&nit);

  //printf("sumlog=%f ssq=%f\n",sumlog,ssq);

  *lik = (double)(sumlog) + (double)n * log((double)ssq);;
  //printf("likelihood is %lf\n",*lik);

  free(alpha); free(beta); free(x); free(thetab);
  free(xnext); free(xrow); free(rbar); free(v);
  free(aa); free(pp); free(resid); free(e);
}
 ...
 int p,q;
{
  ...
  for(i=0;i<p && bad == 0;i++){
	...
	if(fabs(parms[i]) >= d)bad = 1;
	}

  for(i=0;i<q && bad == 0;i++){
	d = (double)choose(q,i + 1);
	if(fabs(parms[p + i]) > d)bad = 1;
	}
